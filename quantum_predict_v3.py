...
class FootballDataFetcher:
    ...
    def get_team_form(self, comp_code, last_n=30):
        """Forma reale separata casa/trasferta + stanchezza (giorni ultima partita)."""
    ...
    def get_h2h(self, team1, team2, comp_code, limit=10):
        """Testa a testa tra due squadre."""
        try:
            results = self.get_past_results(comp_code, limit=50)
            ...
            for r in h2h:
                ...
                ht = r["homeTeam"]["name"]
                if gh > ga:
                    (t1_wins if ht == team1 else t2_wins).__class__  # dummy
...
def main():
    ...
    for league_name, comp_code in COMPETITIONS.items():
        ...
        standings = {}
        try:
            form_map = fetcher.get_team_form(comp_code, last_n=30)
            if form_map:
        ...
        for match in matches[:10]:
            try:
                ...
                # H2H per questa partita
                h2h = None
                try:
                    h2h = fetcher.get_h2h(hname, aname, comp_code, limit=10)
                except Exception:
                    pass
