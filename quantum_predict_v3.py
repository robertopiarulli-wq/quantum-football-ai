...
class FootballDataFetcher:
    ...
    def get_team_form(self, comp_code, last_n=20):
        """Calcola forma reale e xG stimato da ultime N partite per ogni squadra."""
        ...
        for r in results:
            ...
            ht = r["homeTeam"]["name"]
            at = r["awayTeam"]["name"]
            for team, gf, gc in [(ht, gh, ga), (at, ga, gh)]:
                if team not in team_stats:
                    team_stats[team] = {"results": [], "gf": [], "gc": []}
        ...
        for team, s in team_stats.items():
            res = s["results"][-5:]
            form_score = sum(1.0 if r=="W" else 0.5 if r=="D" else 0.0 for r in res) / max(1, len(res))
            xg  = sum(s["gf"]) / max(1, len(s["gf"]))
            xga = sum(s["gc"]) / max(1, len(s["gc"]))
            form_map[team] = {"form": round(form_score,3), "xg": round(xg,2), "xga": round(xga,2)}
        return form_map
...
def extract_features(hname, aname, form_map=None, dynamic_elo=None):
    ...
    # Form e xG reali se disponibili, altrimenti stima da ELO
    hform_data = (form_map or {}).get(hname, {})
    aform_data = (form_map or {}).get(aname, {})

    hxg  = hform_data.get("xg",  2.1 if he>1800 else 1.6 if he>1700 else 1.2)
    axg  = aform_data.get("xg",  2.1 if ae>1800 else 1.6 if ae>1700 else 1.2)
    hxga = hform_data.get("xga", 0.9 if he>1800 else 1.1 if he>1700 else 1.3)
    axga = aform_data.get("xga", 0.9 if ae>1800 else 1.1 if ae>1700 else 1.3)
    hform = hform_data.get("form", 0.55)
    aform = aform_data.get("form", 0.50)
...
def main():
    ...
    for league_name, comp_code in COMPETITIONS.items():
        ...
        # ── FORM REALE dal campionato ────────────────────────
        form_map = {}
        try:
            form_map = fetcher.get_team_form(comp_code, last_n=20)
            if form_map:
                print(f"   📈 Form reale: {len(form_map)} squadre")
        except Exception as e:
        ...
        for match in matches[:10]:
            try:
                ...
                feat = extract_features(hname, aname, form_map=form_map, dynamic_elo=dynamic_elo)
                pred = full_prediction(feat)
                src_h = feat.get("src_home", "ELO")
                src_a = feat.get("src_away", "ELO")
                if src_h == "API" or src_a == "API":
                    print(f"      📡 Dati reali: {hname}({src_h}) {aname}({src_a})")
